...
module Yaks
  class DefaultPolicy
    ...
    #
    # @raise [NameError] occurs when no mapper is found
    ...
    def derive_mapper_from_collection(collection)
      unless collection.respond_to?(:first)
        raise "Failed to find a collection mapper for #{collection.inspect} because it is not a collection."
      end
